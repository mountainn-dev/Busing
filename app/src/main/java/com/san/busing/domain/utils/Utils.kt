...
object Utils {
    fun getRouteType(routeTypeCd: Int) =
        when (routeTypeCd) {
    ...
    fun getPlateType(plateType: Int) =
        when (plateType) {
            NONE.code -> NONE
    ...
    fun getColorByRouteType(type: RouteType) =
        when (type) {
    ...
    fun getLightColorByRouteType(type: RouteType) =
        when (type) {
    ...
    fun getBusImageResourceByRouteType(type: RouteType) =
        when (type) {
    ...
    fun getBackgroundByBookMarkStatus(bookMark: Boolean) =
        when (bookMark) {
